...
function FilterSubsystemByMachine(machine, subsystem) {
  ...
  function fillelement(e) {
    ...
    $('#' + subsystem).empty();
    var options = [];
    $('#' + subsystem).append($("<option></option>").attr("value", -1).text('انتخاب کنید'));
    for (var i = 0; i < data.length; i++) {
      options.push('<option value="' + data[i].SubSystemId + '">' + data[i].SubSystemName + '</option>');
    }
    $('#' + subsystem).append(options);
    $('#' + subsystem).trigger('chosen:updated');
  }
}