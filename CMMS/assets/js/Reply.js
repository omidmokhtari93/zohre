...
function SendDataToDB(btn) {
    ...
    $('#drFail :selected').each(function (index, val) {
        failReason.push({ FailReasonId: val.value });
    });

    $('#drTakhir :selected').each(function (index, val) {
        delayReason.push({ DelayReasonId: val.value });
    });

    $('#drAction :selected').each(function (index, val) {
        action.push({ ActionId: val.value });
    });

    $('#DrstopReason :selected').each(function (index, val) {
        stopReason.push({ StopReasonId: val.value });
    });
