...
var typingTimer;
var doneTypingInterval = 2000;
var $helpinput = $('#txthelpPartsSearch');
$helpinput.on('keyup', function () {
    clearTimeout(typingTimer);
    typingTimer = setTimeout(doneTypinghelp, doneTypingInterval);
    $('#helppartsLoading').show();
    $('#txtSubSearchPart').val('');
    $('#helpgridParts tbody').empty();
    if ($('#txthelpPartsSearch').val() === '') {
        $('#helpPartsSearchResulat').hide();
    }
});
$helpinput.on('keydown', function () {
    clearTimeout(typingTimer);
});
function doneTypinghelp() {
    if (($helpinput).val().length > 2) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/PartsFilter",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify({ 'partName': $helpinput.val() }),
            success: function (e) {
                var tableRows = '';
                var filteredParts = JSON.parse(e.d);
                for (var i = 0; i < filteredParts.length; i++) {
                ...
                $('#helpgridParts tbody').append(tableRows);
                tr = $('#helpgridParts tr').clone();
                $('#helppartsLoading').hide();
                $('#helpPartsSearchResulat').show();
            },
            error: function () {
            }
        });
...
var helppartData = [];
$('#helpgridParts').on('click', 'tr#sel', function () {
    var $row = $(this).find("td");
    var $text = $row.text();
    var $value = $row.attr('partid');
    $('#helpPartsSearchResulat').hide();
    $('#txthelpPartsSearch').val('');
    $('#txthelpPartsSearch').removeAttr('placeholder');
    createPartHelpBadge($text, $value);
});
function createPartHelpBadge(text, val) {
    var badgeHtml = '<div class="PartsBadge" ' +
        'onclick="RemoveParthelpBadge($(this));$(this).remove();">' +
        '<label style="direction:rtl;white-space:nowrap;">' + text + '</label>' +
        '<p style="display:none;">' + val + '</p>' +
        '<span>&times;</span>' +
        '</div>';
    $('#helpPartBadgeArea').append(badgeHtml);
    helppartData.push({ PartName: text, PartId: val });
    $('#txthelpPartsSearch').attr('readonly', 'readonly');
}

function RemoveParthelpBadge(e) {
    $('#txthelpPartsSearch').attr('placeholder', 'جستجو کنید ...');
    $('#txthelpPartsSearch').removeAttr('readonly');
    for (var i = 0; i < helppartData.length; i++) {
        if (helppartData[i].PartId === e.children('p').html()) {
            helppartData.splice(i, 1);
        }
    }
}

$('#txthelpSubSearchPart').keyup(function () {
    var val = $(this).val();
    $('#helpgridParts tbody').empty();
    tr.filter(function (idx, el) {
        return val === '' || $(el).text().indexOf(val) >= 0;
    }).appendTo('#helpgridParts');
});

var ele = $('#no');
$('#helpgridParts').on('mouseenter', 'tr', function () {
    ele.css({ 'background-color': '', 'color': '' });
    ele = $(this);
    ele.css({ 'background-color': '#5186d7', 'color': 'white' });
});
    ...
    if (($input).val().length > 2) {
        $.ajax({
            ...
            success: function (e) {
                ...
                rows = $('#gridPartsResault tr').clone();
            },
            error: function () {
            }
        });
...
var partData = [];
$('#gridPartsResault').on('click', 'tr', function () {
    var $row = $(this).find("td");
    var $text = $row.text();
    var $value = $row.attr('partid');
    $('#PartsSearchResulat').hide();
