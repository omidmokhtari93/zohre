...
$('#drNowunit').on('change', function () {
    if ($('#drNowunit :selected').val() === '-1') {
        $('#drNowLine').empty();
    } else {
        var dataa = [];
        dataa.push({
            url: 'WebService.asmx/FilterLines',
            parameters: [{ unitid: $('#drNowunit :selected').val() }],
            func: fillLineDr
        });
        AjaxCall(dataa);
        function fillLineDr(e) {
            var dr = JSON.parse(e.d);
            var body = [];
            $('#drNowLine').empty();
            body.push('<option value="-1">انتخاب کنید</option>');
            for (var i = 0; i < dr.length; i++) {
                body.push('<option value="' + dr[i].LineId + '">' + dr[i].LineName + '</option>');
            }
            $('#drNowLine').append(body.join(''));
        }
    }
});
$('#drAfterUnit').on('change', function () {
    if ($('#drAfterUnit :selected').val() === '-1') {
        ...
        dataa.push({
            url: 'WebService.asmx/FilterLines',
            parameters: [{ unitid: $('#drAfterUnit :selected').val() }],
            func: fillLineDr
        });
        AjaxCall(dataa);
        function fillLineDr(e) {
            ...
            $('#drAfterLine').append(body.join(''));
        }
    }
});
$('#drNewLocUnit').on('change', function () {
    if ($('#drNewLocUnit :selected').val() === '-1') {
        ...
        dataa.push({
            url: 'WebService.asmx/FilterLines',
            parameters: [{ unitid: $('#drNewLocUnit :selected').val() }],
            func: fillLineDr
        });
        AjaxCall(dataa);
        function fillLineDr(e) {
            ...
            $('#drNewLocLine').append(body.join(''));
        }
    }
});
$('#drRecLocUnit').on('change', function () {
    if ($('#drRecLocUnit :selected').val() === '-1') {
        ...
        dataa.push({
            url: 'WebService.asmx/FilterLines',
            parameters: [{ unitid: $('#drRecLocUnit :selected').val() }],
            func: fillLineDr
        });
        AjaxCall(dataa);
        function fillLineDr(e) {
