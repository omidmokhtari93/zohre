...
namespace CMMS
{
    ...
    public class WebService : System.Web.Services.WebService
    {
        ...
        private readonly SqlConnection _partsConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["anbar"].ConnectionString);
        private const string InitVector = "F4568dgbdfgtt444";
        private const string Key = "rdf48JH4";
		public string Sname = "[192.168.16.25].[Chinigonabad98].[dbo].[CmmsAnbar]";																		   
        ...
        public string SelectId(int mid)
        {
            _cnn.Open();
            ...
            _cnn.Close();
            return string.Join(",", Idarr.Select(x => x.ToString()).ToArray());
        }

        [WebMethod]
        public void BSendMasrafi(int mid, MasrafiMain masrafiMain) //Base Information
        {
            var Idarr = SelectId(mid); // method defined to the top of this method
            _cnn.Open();
            var insertFuel = new SqlCommand(
        ...
        public void BSendGridControli(int mid, List<Controls> controls) //Base Information
        {
            ...
            var chooseIdmachin =
                new SqlCommand("if(select case when max(LEN(code)) = 6 then 6 else 8 end as tool from m_machine)=8" +
                               " begin select id from m_machine where SUBSTRING(code,3,3) =" + mid + " end " +
                               "else begin select id from m_machine where SUBSTRING(code,1,3) =" + mid + " end  ", _cnn);
            var rd = chooseIdmachin.ExecuteReader();
        ...
        public void BSendSubSystem(int mid, List<SubSystems> subSystem) //Base Information
        {
            ...
            var deleteM = new SqlCommand("delete from m_subsystem where Mid in " +
                                         "(select id from m_machine where SUBSTRING(CONVERT(varchar(8), code), 3, 3)= " + mid + " )" +
                                         " and code IS NULL ", _cnn);
            ...
            _cnn.Open();
            List<string> machinelist = new List<string>();
            var machine = new SqlCommand("if(select case when max(LEN(code)) = 6 then 6 else 8 end as tool from m_machine)=8" +
                                         " begin select id from m_machine where SUBSTRING(code,3,3) =" + mid + " end " +
                                         "else begin select id from m_machine where SUBSTRING(code,1,3) =" + mid + " end", _cnn);
            ...
            foreach (var Mid in machinelist)
            {
                foreach (var item in subSystem)
                {
                    ...
                    var insertParts = new SqlCommand(
                        "if(select count(id) from m_subsystem where Mid=" + Mid + " and subId=" + item.SubSystemId + ")=0 " +
                        "begin INSERT INTO [dbo].[m_subsystem](Mid , subId)" +
        ...
        public void BSendGridGhataat(int mid, List<Parts> parts) //Base Information
        {
            ...
            foreach (var Mid in machinelist)
            {
                ...
                {
                    var insertParts = new SqlCommand(
                        "if(select count(id) from m_parts where Mid=" + Mid + " and PartId=" + item.PartId + ")=0 " +
                        "begin INSERT INTO [dbo].[m_parts]([Mid],[PartId],[mYear],[min],[max],[chPeriod],[comment])" +
        ...
        {
            var selectedId = SelectId(mid);
            _cnn.Open();
            var insertInstruct = new SqlCommand(
                "if (select count(Mid) from b_inst where Mid = " + mid + ") <> 0 " +
                "UPDATE [dbo].[m_inst] " +
                "SET[inst] = '" + dastoor + "' " +
                "WHERE Mid in (" + selectedId + ") " +
                " else " +
        ...
        public string GetG(int mid)
        {
            ...
            var getParts =
                new SqlCommand(
                     "SELECT m_parts.id,m_parts.PartId,CmmsAnbar.PartName,mYear,min,max,chPeriod,m_parts.comment,i_measurement.measurement,i_measurement.id as measurid FROM CMMS.dbo.m_parts" +
                    " inner join "+ Sname + " on CmmsAnbar.Serial = m_parts.PartId collate SQL_Latin1_General_CP1256_CI_AS inner join " +
        ...
        public string BGetG(int mid) //get Base information
        {
            ...
            var getParts =
                new SqlCommand(
                    "SELECT id,b_parts.PartId,CmmsAnbar.PartName,mYear,min,max FROM CMMS.dbo.b_parts" +
                    " inner join "+ Sname + " on CmmsAnbar.Serial = b_parts.PartId where Mid = " + mid + " ", _cnn);
            var rd = getParts.ExecuteReader();
            while (rd.Read())
            {
                partsList.AddRange(new List<Parts>
        ...
        public string PartsFilter(string partName)
        {
            ...
            var parts = new SqlCommand("SELECT StockIoDetail.ItemNo as Serial, G.Description as PartName " +
                                       " FROM dbo.StockIoDetail30 AS StockIoDetail LEFT OUTER JOIN " +
                                       " dbo.StockIo30 AS StockIo ON StockIoDetail.StockIoKey = StockIo.AutoKey LEFT OUTER JOIN " +
                                       " dbo.GeneralDesc AS G ON G.Itemcode = StockIoDetail.ItemNo " +
                                       " WHERE (StockIo.Type <> 'T') AND(StockIo.Type <> 'Q') AND(G.Description <> '') AND  G.Description like '%" + farsiPart1 + "%' OR " +
                                       " G.Description like '%" + farsiPart2 + "%' OR G.Description like '%" + partName + "%' or G.Description like N'%" + farsiPart1 + "%' OR " +
                                       " G.Description like N'%" + farsiPart2 + "%' OR G.Description like N'%" + partName + "%' " +
                                       "GROUP BY StockIoDetail.ItemNo, G.Description", _partsConnection);
        ...
        public string ReplyDataFromDb(int requestId)
        {
            ...
            _cnn.Close();
            _cnn.Open();
            var selectParts = new SqlCommand(" SELECT anbar.PartName, dbo.r_tools.count, dbo.i_measurement.measurement AS Measur,case when dbo.r_tools.rptools=1 then 'تعمیر' else 'تعویض' end as rptool " +
                                             " FROM dbo.r_tools INNER JOIN " +
                                             " " + Sname + " as anbar ON dbo.r_tools.tools_id = anbar.Serial INNER JOIN " +
                                             " dbo.i_measurement_part ON dbo.r_tools.tools_id = dbo.i_measurement_part.Serial INNER JOIN " +
                                             " dbo.i_measurement ON dbo.i_measurement_part.measurement = dbo.i_measurement.id " +
                                             " WHERE(dbo.r_tools.id_rep = " + replyId + ") " +
                                             " union all " +
                                             " SELECT anbar.PartName, dbo.r_tools.count, 'عدد' as Measur,case when dbo.r_tools.rptools=1 then 'تعمیر' else 'تعویض' end as rptool " +
                                             " FROM dbo.r_tools INNER JOIN " +
                                             " " + Sname + ".[dbo].[CmmsAnbar] as anbar ON dbo.r_tools.tools_id = anbar.Serial " +
            ...
            _cnn.Close();
            _cnn.Open();
            var selecchangedParts = new SqlCommand("SELECT dbo.subsystem.name as subname,anbar.PartName as partname, m_machine.name as machname " +
                                                   ...
                                                   "inner join " + Sname + " as anbar on dbo.r_helppart.part_id = anbar.Serial " +
        ...
        public string GetMeasurementPartTable()
        {
            ...
            var list = new List<string[]>();
            var array = new List<string[]>();
              var selectAll = new SqlCommand(" SELECT [CMMS].dbo.i_measurement_part.id,CMMS.dbo.i_measurement.[measurement],anbar.PartName,CMMS.dbo.i_measurement.id as mid " +
                                           ...
                                           " inner join " + Sname + " as anbar on anbar.Serial = CMMS.dbo.i_measurement_part.Serial collate SQL_Latin1_General_CP1256_CI_AS" +
        ...
        public string MojoodiMachinePart(int machineid)
        {
            ...
            var getMojoodi = new SqlCommand("SELECT dbo.m_parts.PartId, anbar.PartName, anbar.Qty " +
                                            "FROM dbo.m_machine INNER JOIN " +
                                            "dbo.m_parts ON dbo.m_machine.id = dbo.m_parts.Mid INNER JOIN " +
                                            "" + Sname + " as anbar ON dbo.m_parts.PartId = anbar.Serial collate SQL_Latin1_General_CP1256_CI_AS " +
                                            "WHERE(dbo.m_machine.id = " + machineid + ")", _cnn);
            var rd = getMojoodi.ExecuteReader();
            while (rd.Read())
            {
                partlist.Add(new []{rd["PartName"].ToString() ,rd["PartId"].ToString(), rd["Qty"].ToString() });
            }
        ...
        [WebMethod]
        public string MojoodiAnbar(string partid)
        {
            _cnn.Open();
            var list = new List<string[]>();
            var partlist = new List<string[]>();
            var getAnbar = new SqlCommand("SELECT [Serial],[PartName],[Qty] FROM " + Sname + " where Serial = " + partid+" ", _cnn);
        ...
        public string FilterGridCm(string s , string e)
        {
            ...
            var filter = new SqlCommand("SELECT Forecast.tarikh, anbar.PartName,anbar.Qty, dbo.i_units.unit_name, dbo.m_machine.name, dbo.m_machine.code " +
                                        ...
                                        ""+Sname+" AS anbar ON Forecast.PartId = anbar.Serial ON dbo.m_parts.id = Forecast.m_partId " +
            ...
            while (r.Read())
            {
                list.Add(new []{ r["unit_name"].ToString(), r["name"].ToString() , r["code"].ToString() ,
                    r["PartName"].ToString() , r["tarikh"].ToString(),r["Qty"].ToString() });
            }
            listt.AddRange(list);
            _cnn.Close();
        ...
        public string GetRequestedParts(int requestId)
        {
            ...
            _cnn.Close();
            _cnn.Open();
            var getparts = new SqlCommand("SELECT anbar.PartName as name, anbar.Serial as id FROM CMMS.dbo.r_reqwaitpart as main " +
                                          "inner join "+Sname+" as anbar on main.part_id = anbar.Serial " +
                                          "inner join CMMS.dbo.r_partwait as req on main.id_wait = req.id " +
                                          "where req.req_id = "+requestId+" ",_cnn);
            var re = getparts.ExecuteReader();
            while (re.Read())
            {
                parts.Add(new Parts(){ PartId = re["id"].ToString(), PartName = re["name"].ToString()});
            }
            _cnn.Close();
            var obj = new
            {
